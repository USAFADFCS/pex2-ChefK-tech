...
void* SJFcpu(void* param) {
    ...
    Process* p = NULL;  // DONE: uncomment when you implement this function
    ...
    while (1) {
        ...
        if (p == NULL) {
            ...
            // Same code as FIFO except we now select which process to run
            // based on the shortest burst time remainingc
...
void* NPPcpu(void* param) {
    ...
    while (1) {
        ...
        if (p == NULL) {
            ...
            // Same code as FIFO and SJF except we now select which process to run
            // based on the which one has the highest priority
